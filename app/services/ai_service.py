...
def generate_practice_questions(attempts):
    """
    ...
    example_json = '''
{
    ...
    prompt = {
        "role": "user",
        "content": f"""Generate a set of math questions for a student based on their performance history.
...
6. Questions should be in these formats:
   - Addition: "23 + 45"
...
Return ONLY a JSON object with question types as keys and questions as values, like this example:
{example_json}"""
    }

    logger.debug("Sending prompt to OpenAI")
    logger.debug(f"Prompt context - Weak areas: {len(weak_areas)}, Strong areas: {len(strong_areas)}")
